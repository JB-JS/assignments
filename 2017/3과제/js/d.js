  ...
  open: function (database, table) {
    ...
    request.onupgradeneeded = function (e) {
      ...
      $.each(table, function (name, option) {
        if (option == null) option = { keyPath: 'idx', autoIncrement: true }
...
var Folder = {
    ...
    $('.folder-ul li')
      ...
      .droppable({
        drop: function (e, ui) {
          ...
          if (folderName != '..') {
            overlap = Folder.overlapChk(
              Folder.nowChildren[folderName].children,
              dragName
            )
          } else {
  ...
  tabAppend: function (tab, idx) {
    ...
    $tab.attr('idx', idx).find('.editor-text').html(tab.name)

    if (tab.active) {
      $('.editor-input').attr('readonly', false).val(tab.tmpContent)
  ...
  hook: function () {
    ...
    $(document).on(
      {
        dblclick: function (e) {
          ...
          window.open(
            URL.createObjectURL(Folder.nowChildren[name].file),
            '이미지 보기'
          )
        },
      },
    ...
    $(document).on(
      {
        dblclick: function () {
          if ($(this).hasClass('image-li')) return false

          var file = Folder.nowChildren[$(this).attr('name')]
          var error = false

          $.each(Folder.tabs, function (idx, tab) {
            if (file.route == undefined) return false

            if (
              file.route.join('/') + file.name ==
              tab.route.join('/') + tab.name
            ) {
              error = true
          ...
          file.tab = true
          file.order = Folder.tabs[Folder.tabs.length - 1]
            ? Folder.tabs[Folder.tabs.length - 1].order + 1
            : 0
    ...
    $(document).on(
      {
        click: function () {
          ...
          $('.editor-input').val(tab.tmpContent).attr('readonly', false)
      ...
      keydown: function (e) {
        if (e.keyCode == 9) {
          ...
          if (start != end) {
            ...
            line = line.join('\n')
            this.value =
              this.value.slice(0, start) + line + this.value.slice(end)
    ...
    $(document).on(
      {
        click: function (e) {
          ...
          if (tab.content != tab.tmpContent) {
            if (
              confirm(
                '편집중인 파일이 저장이 안되어 있습니다. 저장지 않을시 내용이 손실될 수 있습니다. 저장하시겠습니까?'
              )
            ) {
          ...
          if (tab.active) {
            tab.active = false
            $('.editor-input').attr('readonly', true).val('')
